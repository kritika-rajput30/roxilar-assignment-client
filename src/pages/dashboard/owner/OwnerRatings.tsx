  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        ...
        // Flatten all ratings across all stores
        const allRatings = stores.flatMap((store: any) =>
          (store.ratings || []).map((rating: any) => ({
            ...
            store: {
              name: store.name,
              address: store.address, // Optional: if address is in the store
            },
            user: rating.user,
          }))
        );
      ...
      {ratings.map((rating) => (
        ...
        >
          ...
          <div className="flex items-center mt-2 space-x-2">
            <span className="text-yellow-500 font-bold">Rating: {rating.rating}★</span>
          </div>
          {rating.comment && (
            <p className="mt-2 italic text-gray-700">“{rating.comment}”</p>
          )}
           {rating.user && (
            <p className="mt-2 italic text-primary-dark">By {rating.user.email}</p>
          )}
